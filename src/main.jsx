...
const WEBSITE_CATALOG = [
  { id: "tunnelSolareItalia", flag: "🇮🇹", name: "Tunnel Solare Italia", country: "Italia", language: "Italiano", domain: "https://www.tunnelsolare.net", homeH1: "IL TUNNEL SOLARE, SISTEMI DI ILLUMINAZIONE NATURALE", homeTitle: "Tunnel Solare | Sistemi di illuminazione naturale", homeDescription: "Tunnel solari e lucernari per illuminare gli spazi interni con luce naturale.", shopH1: "NEGOZIO ONLINE DI TUNNEL SOLARE", shopTitle: "Negozio online di tunnel solare | Tunnel Solare", shopDescription: "Acquista online tunnel solari, lucernari solari e accessori per luce naturale." },
  { id: "tuboSolarEspana", flag: "🇪🇸", name: "Tubo Solar España", country: "España", language: "Español", domain: "https://www.tubosolar.net" },
  { id: "tageslichtspotAlemania", flag: "🇩🇪", name: "Tageslichtspot Alemania", country: "Alemania", language: "Alemán", domain: "https://www.tegeslichtspot.com" },
  { id: "puitDeLumiereFrancia", flag: "🇫🇷", name: "Puit de Lumière Francia", country: "Francia", language: "Francés", domain: "https://www.puitdelumiere.net" },
  { id: "daglichtbuisHolanda", flag: "🇳🇱", name: "Daglichtbuis Holanda", country: "Holanda", language: "Neerlandés", domain: "https://www.daglichtbuis.net" }
];
...
function WebsitesView({ token }) {
  ...
  const websitePicker = <div className="website-picker workspace-picker">{WEBSITE_CATALOG.map((site) => <button type="button" key={site.id} className={site.id === selectedWebsite.id ? "active" : ""} onClick={() => setSelectedWebsiteId(site.id)}><span>{site.flag} {site.country}</span><small>{site.domain.replace("https://", "")}</small></button>)}</div>;
