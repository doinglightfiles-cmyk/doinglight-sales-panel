...
- El área **Webs → SEO** permite a administradores editar y guardar el *page title*, metadescripción, H1 y slug por dominio; **Webs → Redirecciones** conserva también origen/destino por dominio. Se han dado de alta Italia, España, Alemania, Francia y Holanda. La persistencia usa la migración `031_websites_settings.sql` y el bloque `websites` de ajustes.
...
- Los nombres visibles de las cuentas internas se fijan por correo para evitar que la configuración de Railway las vuelva a presentar como «Administración Doinglight»: Edu, Laura, JV, Ángel, Roberto, Claudine, María, Holger y Almacén.
- El chat incorpora «Superpoderes» con destinatario individual. La primera invocación, «Enviar rayo de luz ultravioleta», activa un contador propio y transforma temporalmente los acentos verdes del panel del destinatario en una alerta roja pulsante hasta que abra el chat.
- Las listas de compra de almacén admiten borradores persistentes: pueden guardarse, cerrarse y reabrirse para continuar editándolas. Solo la acción «Enviar lista de la compra» las bloquea y genera los avisos para Marketing y JV.
- El chat muestra en una columna lateral todos los usuarios activos del sistema. El grupo original se mantiene como «Chat general», mientras que cada usuario abre una conversación privada visible únicamente para sus dos participantes y conserva su propio contador de mensajes pendientes.
...
- `almacen@doinglight.es` está configurado y activo mediante la lista de usuarios de Railway. Dispone de una interfaz exclusiva con cabecera verde, cola de albaranes por fecha de entrada, detalle de líneas y acciones «PEDIDO FINALIZADO» e «INCIDENCIA». Los pedidos preparados se colorean en verde, las incidencias en rojo, y ambas acciones notifican a Administración, Marketing y J. V. Tarancón.
